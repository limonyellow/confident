...
class MapSourceLoader(SourceLoader):
    ...
    def load_fields(self, settings: BaseSettings) -> List[ConfigField]:
        ...
        if map_field is not None and map_name is not None:
            raise ValueError(
                "Cannot have both `map_field` and `map_name`. Only one can be used."
            )
        ...
        if map_field:
            ...
            selected_config = config_map.get(map_name)  # type: ignore[union-attr]

        if selected_config is None:
            raise KeyError(
                f"No matching map config to {map_name=}. Check your `config_map`."
            )
            ...
            config_fields.append(
                ConfigField(
                    ...
                    value=convert_field_value(
                        settings=settings, field_name=name, origin_value=value
                    ),
                    origin_value=value,
